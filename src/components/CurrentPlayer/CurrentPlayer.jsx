...
import {
  ...
  playSong,
  togglePlaying,
} from "../../features/currentSong/currentSongSlice";
...
const CurrentPlayer = ({ songs, currentSong }) => {
  ...
  useEffect(() => {
    ...
    const playAudio = () => {
      if (
        ...
        !currentSong.audio.ended
      ) {
        currentSong.audio.pause();
      }

      if (currentSong.audio) {
        currentSong.audio.play().catch((error) => {
    ...
    if (currentSong.playAll !== true) {
      ...
      timeInterval = setInterval(() => {
        setPosition(currentSong.audio?.currentTime || 0);
      }, 1000);
    } else {
      playAudio();
      timeInterval = setInterval(() => {
  ...
  const playMusic = async (fc) => {
    const song = fc(songs, currentSong.song);
    if (currentSong?.audio) {
      currentSong.audio.pause();
    }
    await dispatch(playSong({ song }));
  };
        ...
        <Widget>
          ...
          <Box sx={{ display: "flex", alignItems: "center" }}>
            ...
            <Box sx={{ ml: 1.5, minWidth: 0 }}>
              ...
              <Typography noWrap>{currentSong.song.title}</Typography>
            </Box>
          </Box>
          <Slider
          ...
          >
            <span onClick={() => playMusic(findPreviousSong)}>
              <IconButton aria-label="previous song">
            ...
            <span onClick={() => playMusic(findNextSong)}>
